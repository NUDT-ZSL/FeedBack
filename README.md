...
tests/
  test_ledger.py   # 46 个验收测试,逐条对应需求 1-8 及两项收紧行为
```
...
| 3 | 可注入逻辑时钟;重复上报幂等;矛盾来源保留 + 可读冲突记录 | `MetricLedger(clock=...)`;完全相同的 (字段,时刻,来源,值) 为幂等空操作;同字段同时刻不同值全部保留于 `conflicts()`,该时刻按缺失(冲突)参与计算;**冲突中若新上报与某一既有来源一致,判定已消解**:采信被一致认可的值,来源链只挂被采信来源,记录标注 `resolved` 与消解时刻;都不一致则追加来源、冲突保留;消解后再遇矛盾则重开并保留消解历史 |
...
| 7 | 口径切换轨迹,差异量化归因到具体依赖 | `trajectory(metric, start, end)`:每次切换给出生效时刻、前后版本/公式、同一时刻按旧/新口径的取值与差值;归因逐依赖给出 `added/removed/retained` 状态与**数值贡献差**(`contribution_diff`),各依赖贡献差之和精确等于切换前后取值差(`attribution_sum == diff`),纯常量/结构变化归入 `<formula_structure>` 项 |
...
- **贡献差归因**:切换差异通过新旧公式 AST 对齐切分为"变化区域",按区域顺序
  逐个切换形成 telescoping 序列;区域内用留一法(将该依赖引用置零)量化移除 /
  新增依赖的贡献,保留依赖分摊区域残余,纯常量变化归入 `<formula_structure>` 项。
  数学上保证各依赖贡献差之和精确等于切换前后取值差,且与人工逐步推导一致。
