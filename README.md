...
（它在规则起始版本尚不存在、在目标版本首次存活）；若目标名在起始版本已是独立存活字段，
构成两个字段身份合并，登记/载入都会被拒绝，错误同时指出冲突的源字段名与目标名。
...
路径中的版本一律规范化（右侧补 0 的段去掉），因此查询 `1.0→3.0` 与 `1.0.0→3.0.0`
产出**逐字符相同**的路径字符串。
...
1. **结构校验**：JSON 结构、五个顶层段（`devices/fields/migration_rules/configs/adaptations`，
   缺一即拦，不当空集合）、必填键、基本类型、重复标识、分叉规则，错误带 JSON 路径
   （如 `$.devices[0].firmware`），抛 `CorruptStateError`；JSON 截断等解析错误带行列位置；
2. **语义重放**：在全新临时内核上重放全部登记（复用内核全部校验：版本合法、引用字段存在、
   改名不得合并两个字段身份、无环等），再把每条适配记录**重新计算并与存档逐条比对**
   （含记录 ID）。

语义重放阶段产生的校验异常**原样抛出**（`VersionError` / `ValidationError` /
`DuplicateError` / `MigrationChainError` / `MissingFieldError`），保留最原始的原因、
异常类型与出错位置，不做二次包装；只有结构问题或「重算记录与存档不一致」（篡改）
才抛 `CorruptStateError`。
