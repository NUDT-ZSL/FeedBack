...
- **导出 / 导入**（格式版本 1，见 `serde.py`）：
  `kernel.export()` 导出纯 JSON 兼容字典（属性模式、默认效果、全部请求、
  全部策略版本及规则全字段、归因配置），`ImpactKernel.import_data(data)`
  或 `kernel_from_json(text)` 重建。往返后重放、决策依据、规则命中与归因
  结果与导出前一致；损坏或字段缺失的输入以带位置的 `ValidationError`
  拒绝，且导入失败不改动任何已有内核的内存状态。
- **法医式归因接口**：`explain_flip(old, new, request, diffs=...)` 的
  `diffs` 参数是公开接口，用于差异清单来自外部系统的场景（含空清单），
  可在不触碰内部字段的情况下复现"差异为空却仍翻转"的无法归因分支。
...
| `serde.py` | 导出 / 导入（格式版本 1，可往返，损坏输入清晰拒绝） |
