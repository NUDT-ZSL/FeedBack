...
5. **汇合合并**：路径以任意顺序到达汇合点（`arrive_at`），合并结论与到达顺序无关；同变量不同值保留双方来源并生成可读冲突记录，未解决前生效值恒为字典序最小分支的值，记录与状态自洽（`narrative/merge.py`、`narrative/engine.py`）。
6. **存档演进**：格式版本迁移链 + 内容演进（新变量补默认值、移除节点标记不可用），迁移后进度与分支归属不变（`narrative/migration.py`）。
7. **查询接口**：进入条件 / 可达后继 / 变量来源链 / 分支归属 / 未解决冲突 / 变量取值状态（`variable_status`），全部按稳定顺序返回。
