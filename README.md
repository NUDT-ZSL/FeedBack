...
- **接收方待投递队列本身在转移完成后按投递全序重排**（priority 降序、
  produced_at 升序、msg_id 字典序），与该消息直接发布/投递给它时的顺序
  完全一致——即使原队列里已有消息、或多条消息同优先级同 produced_at
  仅 msg_id 不同，顺序也稳定不乱；快照里的 pending 顺序同样是规范序；
...
- 重投递计数按**批次**（一次掉线 / 一次命中的超时扫描各计 1，多轮
  掉线/超时循环既不产生重复队列条目也不重复计数）；专项覆盖一次掉线
  跨多主题多条消息只计 1 个批次、再次掉线才计第 2 个、超时回队后无在途
  消息再掉线不另计批次、超时与掉线在重新投出后是两个独立批次；
...
- 注销 cleanup（含旧名 drop 别名与旧快照归一）/ transfer（在线同主题
  未满接收、**转移后接收方物理队列即按投递全序排列**、同优先级同
  produced_at 仅 msg_id 不同时稳定按字典序、与“直接投递给接收方”
  顺序逐元素一致、接收方原有低优先级消息被正确越过、曾投出/未投出的
  redelivered 区分、离线/满队列/已持有 retained、无处可去计入清理）、
  计数跨注销累计、消息本体 GC 防泄漏；
