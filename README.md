...
3. **中段缺失**：对每个相邻锚点分界取两侧局部窗口（各最多 3 点）各自 OLS，
   当两直线在分界处的**基准轴错位量** ≥ `min_cut_gap_ms` 且错位/窗口残差
   的信噪比 ≥ `cut_residual_factor` 时判定切口。缺失区间优先取"两侧锚点间
   缺失的基准条目"时间并集（可逐条复核），两侧分别拟合，**缺失绝不进入
   速率拟合**——避免被误当成内容压缩。
