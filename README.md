...
5. **删除 / 拆分只重算受影响边**：操作仅拆除 / 重定向与该条目相关的引用边，返回受
   ...
   * **旧快照兼容**：快照中存有权威的拆分事件账 `splits`。早期没有 `status` /
     `splits` 字段的旧快照，导入时会结合父条目 `split_into`、子分片 `split_from`
     回指**推断并回填归档状态**，使导入结果与当前格式导出再导入完全一致；拆分来源缺失
     或指向不存在条目、active 却声明分片等矛盾仍按损坏（`CorruptSnapshot`）拒绝。
   * **归档只读**：归档条目上的修订 / 集成 / 解决冲突 / 加引用一律被拒绝，错误信息说明
     该条目已归档及其延续分片，且拒绝后版本号、修订链、逻辑时钟均不改变（不会长出并行
     版本线）。
