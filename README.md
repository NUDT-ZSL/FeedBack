...
请求 `n` 个令牌时沿路径**自叶向根**判定，每一层都必须为整笔 `n` 独立记账：

1. 先用本层**稳态桶**全额垫付直接子节点的借入需求（突发桶不外借）；
2. 垫付后，本层再支付 `n − 垫付额`：依次用剩余稳态桶、本层突发桶；
3. 仍不足 → 向直接父节点借入。**借入即耗，不进本层桶，不得转贷**；
4. 根无父可借；任一层凑不齐 → 整笔拒绝，**不落任何账**，返回从请求节点
   一路到根的原因链（`ok / insufficient / disabled / not_evaluated`）。

因为父节点只能借出自己稳态桶里真实存在的令牌，且借入令牌不得转贷，同一份
令牌不可能被借出两次。

### 核心不变量（每个操作后测试都会校验）

- `0 <= steady`，且 `steady + 未还借出 <= capacity`（剩余加借出不超过容量）；
- 任一父节点：所有直接子节点该层已用之和 `<=` 父级总额度
...
稳态速率；子孙的突发再大也要受祖先链路约束（祖先凑不齐整笔即拒绝）。
