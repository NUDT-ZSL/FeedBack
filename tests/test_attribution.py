...
class ObservationTests(unittest.TestCase):
    ...
    def test_idempotent_same_value(self):
        ...
        self.assertIsNone(self.eng.observe("s", 1, "x", 5.0))
        self.assertEqual(self.eng.duplicate_observations(), ())

    def test_conflicting_duplicate_keeps_first(self):
        self.eng.observe("s", 1, "x", 5.0)
        dup = self.eng.observe("s", 1, "x", 6.0)
        self.assertIsNotNone(dup)
        self.assertEqual((dup.kept_value, dup.rejected_value), (5.0, 6.0))
        self.assertEqual(self.eng.value_at("s", 1, "x"), 5.0)
        self.assertEqual(len(self.eng.duplicate_observations()), 1)
