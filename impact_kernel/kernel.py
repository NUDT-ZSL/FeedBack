...
from .explain import Explanation, explain_flip
...
class ImpactKernel:
    ...
    def _policy(self, version: str) -> Policy:
    ...
    def _request(self, request_id: str) -> AccessRequest:
    ...
    def decision_basis(self, request_id: str, version: str = "new") -> DecisionTrace:
        """查询任意请求在指定策略下的决策依据。"""
        return evaluate(self._policy(version), self._request(request_id))

    def replay(self, old_version: str = "old", new_version: str = "new") -> ReplayResult:
        """对全部请求分别用两套策略重放，结果按请求标识字典序返回。"""
        return replay(
            self._policy(old_version),
            self._policy(new_version),
    ...
    def policy_diff(self, old_version: str = "old",
        ...
        return diff_policies(self._policy(old_version), self._policy(new_version))
